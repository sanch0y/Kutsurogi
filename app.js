...
class UI {
  ...
  getBagButtons() {
    ...
    buttons.forEach(button => {
      ...
      if (inCart) {
        button.innerText = "カートに入っています";
        button.disabled = true;
      }
      button.addEventListener('click', event => {
        event.target.innerText = "カートに入っています";
  ...
  addCartItem(item) {
    ...
    <div>
      <h4>${item.title}</h4>
      <h5>${item.price}¥</h5>
      <span class="remove-item" data-id=${item.id}>削除</span>
    </div>
    <div class="">
  ...
  removeItem(id) {
    ...
    button.innerHTML = `<i class="fas fa-cart-arrow-down"></i>カートに入れる`;
  }
